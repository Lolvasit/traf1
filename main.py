...
setting: Setting = Setting.get_or_none(id=1)


if setting is None:
    set_id = Setting.insert().execute()
    setting = Setting.get(id=set_id)
    capcha_btn_apply = setting.capcha_btn_apply
    capcha_btn_decline = setting.capcha_btn_decline
...
async def join_request(message: types.Message):
    ...
    #    await send_start_msg(user_id, 1)
    #     return
    ...
    text = setting.capcha_text.format(
        id=user.id, username=user.username,fname=user.first_name,lname=user.last_name,
        fullname = user.full_name,anyname=user.username if not (user.username == None) else user.full_name,ctitle = ctitle)

    message_data = await bot.send_message(message.from_user.id, text, reply_markup=get_capcha_markup())
...
async def resend_capcha(message_data, user_id):
    setting: Setting = Setting.get(id=1)
    await asyncio.sleep(setting.capcha_time)
    try:
...
class VerificationSet(StatesGroup):
    menu = State()
    msg = State()
    capcha_btn_yes = State()
    capcha_btn_no = State()
    capcha_msg = State()
    resend_time = State()


@dp.callback_query_handler(text="settings_capcha", is_admin=True )
async def _verification_set(callback: CallbackQuery, state: FSMContext):
    await bot.send_message(callback.from_user.id,"Настройка капчи", reply_markup=get_capchaset_markup())

@dp.callback_query_handler(text="capcha_text")
async def _verification_set(callback: CallbackQuery, state: FSMContext):
    await VerificationSet.capcha_msg.set()
    await bot.send_message(callback.from_user.id, "Введите сообщение капчи")

@dp.message_handler(state=VerificationSet.capcha_msg)
async def _verification_set(message: types.Message, state: FSMContext):
    setting: Setting = Setting.get(id=1)
    setting.capcha_text = message.text
    setting.save()
    await state.finish()
    await message.answer("Готово")

@dp.callback_query_handler(text="capcha_btn_apply")
async def _verification_set(callback: CallbackQuery, state: FSMContext):
    await VerificationSet.capcha_btn_yes.set()
    await bot.send_message(callback.from_user.id, "Введите что будет отображаться на конопке принятия")

@dp.message_handler(state=VerificationSet.capcha_btn_yes)
async def _verification_set(message: types.Message, state: FSMContext):
    setting: Setting = Setting.get(id=1)
    setting.capcha_btn_apply = message.text
    setting.save()
    global capcha_btn_apply
    capcha_btn_apply = message.text
    await state.finish()
    await message.answer("Готово")

@dp.callback_query_handler(text="capcha_btn_decline")
async def _verification_set(callback: CallbackQuery, state: FSMContext):
    await VerificationSet.capcha_btn_no.set()
    await bot.send_message(callback.from_user.id, "Введите что будет отображаться на конопке отмены")

@dp.message_handler(state=VerificationSet.capcha_btn_no)
async def _verification_set(message: types.Message, state: FSMContext):
    setting: Setting = Setting.get(id=1)
    setting.capcha_btn_decline = message.text
    setting.save()
    global capcha_btn_decline
    capcha_btn_decline = message.text
    await state.finish()
    await message.answer("Готово")

@dp.callback_query_handler(text="capcha_time")
async def _verification_set(callback: CallbackQuery, state: FSMContext):
    await VerificationSet.resend_time.set()
    await bot.send_message(callback.from_user.id, "Введите время повтора капчи")
...
async def _verification_set(message: types.Message, state: FSMContext):
    setting: Setting = Setting.get(id=1)
    try:
        setting.capcha_time = int(message.text)
        setting.save()
    except:
...
def get_capcha_markup():
    ...
    markup.row(KeyboardButton(setting.capcha_btn_apply),KeyboardButton(setting.capcha_btn_decline))
    return markup
...
def get_admin_markup():
    ...
    markup.add(InlineKeyboardButton("Настройка капчи ♑", callback_data="settings_capcha"))
    return markup

def get_capchaset_markup():
    markup = InlineKeyboardMarkup(row_width=1)
    markup.add(InlineKeyboardButton("Изменить текст капчи 📝", callback_data="capcha_text"))
    markup.add(InlineKeyboardButton("Изменить время капчи 🕒", callback_data="capcha_time"))
    markup.add(InlineKeyboardButton("Изменить текст кнопки ✅", callback_data="capcha_btn_apply"))
    markup.add(InlineKeyboardButton("Изменить текст кнопки ❌", callback_data="capcha_btn_decline"))
    return markup
...
async def menu_msg(num, message, state, call_user_id):
    ...
    try:
        ...
        ctitle = ""
        try:
            ctitle = (await bot.get_chat(users_verifying[message.from_user.id])).title
        except:
            print("")

        msg = setting.start_msg.format(
            id=user.id, username=user.username, fname=user.first_name, lname=user.last_name,
            fullname=user.full_name, anyname=user.username if not (user.username is None) else user.full_name,
            ctitle=ctitle)
        ...
        # await bot.copy_message(call_user_id, setting.start_from_user_id, setting.start_msg, reply_markup=start_kb)
    except:
...
async def _confirm_make_mail(message: Message, state: FSMContext):
    ...
    setting.start_msg=message.text
    setting.save()
    await StartMailingStates.menu.set()
...
async def _process_change_kb_end(message: Message, state: FSMContext):
    ...
    try:
        ...
        msg = setting.start_msg.format(
            id=user.id, username=user.username,fname=user.first_name,lname=user.last_name,
            fullname = user.full_name,anyname=user.username if not (user.username is None) else user.full_name,ctitle = ctitle)
        await bot.send_message(
            message.chat.id,
            msg,
            reply_markup=kb
        )
        await bot.copy_message(message.from_user.id, setting.start_from_user_id, setting.start_msg, reply_markup=kb)
    except:
        await message.answer("Неправильный формат")
        return
    await message.answer("Успешно изменено!")
...
async def send_start_msg(send_to, setting_num):
    ...
    if setting is None or not setting.start_msg:
    ...
    try:
        ...
        ctitle = await bot.get_chat(users_verifying[send_to])
        msg = setting.start_msg.format(
            id=send_to, username=user.username,fname=user.first_name,lname=user.last_name,
            fullname = user.full_name,anyname=user.username if not (user.username is None) else user.full_name,ctitle = ctitle.title)
...
async def _apply(message: Message):
    ...
    if message.text == capcha_btn_apply:
        try:
            loop.create_task(bot.delete_message(message.from_user.id, message.message_id))
            loop.create_task(bot.delete_message(message.from_user.id, message.message_id-1))
    ...
    elif message.text == capcha_btn_decline:
    ...
    await loop.create_task(send_start_msg(user_id, 1))
    del users_verifying[user_id]
